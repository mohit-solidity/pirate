      ...
      <section className="min-h-[85vh] flex flex-col justify-center py-20 md:py-32">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          <div className="lg:col-span-8 space-y-10">
            ...
            >
              Bridging the gap between complex blockchain code and viral
              community adoption through strategic growth architecture and
              secure moderation systems.
            </motion.p>
            ...
            >
              <Magnetic strength={0.1}>
                <Button
                  ...
                  variant="primary"
                >
                  View Dev Work <ArrowRight className="ml-3 w-4 h-4" />
                </Button>
              </Magnetic>
              <Magnetic strength={0.1}>
                <Button
                  onClick={() => navigate("/community")}
                  className="h-16 px-12 text-[12px] uppercase font-bold tracking-widest shadow-2lx shadow-[var(--color-primary)]/20 border-2 border-[var(--color-primary)] bg-[var(--color-primary)] text-black hover:scale-105"
                  variant="primary"
                >
      ...
      <section className="py-32">
        <div className="flex flex-col md:flex-row items-end justify-between mb-24 gap-6">
          ...
          >
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-[10px] font-bold text-[var(--color-primary)] uppercase tracking-[0.5em]">
                Skillset Matrix
              </span>
            </div>
            <h2 className="text-[var(--fs-2xl)] font-extrabold uppercase tracking-tighter">
              Expertise.
            </h2>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {skillCategories.map((cat, i) => (
            <div
              key={i}
              className="glass-card p-10 hover:border-[var(--color-primary)]/30 transition-all duration-500"
            >
              <div
                className="p-4 rounded-2xl mb-10 w-fit bg-white/5 border border-white/5"
                style={{ color: cat.accent }}
              >
                <cat.icon size={24} />
              </div>
              <h3 className="text-[var(--fs-base)] font-bold uppercase tracking-tight mb-8">
                {cat.title}
              </h3>
              <ul className="space-y-4">
                {cat.skills.map((skill, si) => (
                  <li
                    key={si}
                    className="flex items-center gap-4 text-[13px] text-[var(--color-text-muted)] font-medium"
                  >
